...
public class HeavierCore extends Block implements Waterloggable {
      public static final MapCodec<HeavierCore> CODEC = createCodec(HeavierCore::new);
   private static final VoxelShape OUTLINE_SHAPE = Block.createCuboidShape(4.0, 0.0, 4.0, 12.0, 8.0, 12.0);

   public HeavierCore(AbstractBlock.Settings settings) {
      super(settings);
      this.setDefaultState((BlockState)((BlockState)this.stateManager.getDefaultState()).with(Properties.WATERLOGGED, false));
   }

   public MapCodec<HeavierCore> getCodec() {
      return CODEC;
   }
